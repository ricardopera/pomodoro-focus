...
function updateTrayTitle(timeLeft) {
  if (!tray) return;
...
function showTimerCompleteNotification(sessionType) {
  const settings = getSettings();
...
function getNotificationContent(sessionType) {
  const settings = getSettings();
  switch (sessionType) {
    case "focus":
      return {
        title: "\u{1F345} Sess\xE3o de Foco Conclu\xEDda!",
        body: settings.autoStartBreaks ? "Excelente trabalho! A pausa iniciar\xE1 automaticamente." : "Excelente trabalho! Hora de fazer uma pausa."
      };
    case "break-short":
      return {
        title: "\u2615 Pausa Curta Conclu\xEDda!",
        body: settings.autoStartFocus ? "Pausa terminada. Pr\xF3xima sess\xE3o de foco iniciar\xE1 automaticamente." : "Pausa terminada. Pronto para a pr\xF3xima sess\xE3o de foco?"
      };
    case "break-long":
      return {
        title: "\u{1F31F} Pausa Longa Conclu\xEDda!",
        body: settings.autoStartFocus ? "\xD3timo descanso! Pr\xF3xima sess\xE3o de foco iniciar\xE1 automaticamente." : "\xD3timo descanso! Pronto para retomar o foco?"
      };
    default:
      return {
        title: "Timer Conclu\xEDdo",
        body: "O timer foi conclu\xEDdo."
      };
  }
}
...
import_electron4.app.commandLine.appendSwitch("enable-logging");
import_electron4.app.commandLine.appendSwitch("v", "1");
...
import_electron4.app.on("quit", () => {
  console.log("[MAIN] quit event");
  import_electron4.ipcMain.removeAllListeners();
});
function createWindow() {
  console.log("[MAIN] Creating window...");
  const settings = getSettings();
  console.log("[MAIN] Settings loaded:", settings);
  mainWindow3 = new import_electron4.BrowserWindow({
    width: 400,
    height: 600,
    minWidth: 350,
    minHeight: 500,
    ...
    webPreferences: {
      ...
      sandbox: false
      // Disabled for debugging - may cause issues on Windows
    }
    // show: false, // Show immediately for debugging
  });
  console.log("[MAIN] Loading URL...");
  if (false) {
    mainWindow3.loadURL(process.env.ELECTRON_RENDERER_URL);
    mainWindow3.webContents.openDevTools();
  } else {
    mainWindow3.loadFile(import_path.default.join(__dirname, "../../test.html"));
